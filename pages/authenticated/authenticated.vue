<template>
	...
	<scroll-view scroll-y="true" class="container">
		<view class="stores" v-for="(item,index) in storeList">
			...
			<button class="flex-item3" @click="renZheng(item.id)">取消<br/>认证</button>
		</view>
	</scroll-view>
</template>
	...
	export default {
		...
		methods:{
			async getStoreList() {
				...
				const {data: res} = await uni.$http.get('/shop_renzheng/done',params)
				//请求失败
				if(res.status == 200) {
			...
			async renZheng(id) {
				...
				const {data: res} = await uni.$http.post('/shop_renzheng/done',data);
				console.log(res)
					if (res.status == '') {
						return uni.showToast({
							title: '取消认证失败',
